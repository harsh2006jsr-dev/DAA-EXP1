...
// Parameterized Strassen with configurable cutoff threshold
void strassen_hybrid(int **A, int **B, int **C, int n, int cutoff) {
    // Cutoff switch to traditional algorithm
    if (n <= cutoff) {
    ...
    int **M1 = allocate_matrix(k); strassen_hybrid(A_tmp, B_tmp, M1, k, cutoff);

    add_matrix(A21, A22, A_tmp, k);
    int **M2 = allocate_matrix(k); strassen_hybrid(A_tmp, B11, M2, k, cutoff);

    subtract_matrix(B12, B22, B_tmp, k);
    int **M3 = allocate_matrix(k); strassen_hybrid(A11, B_tmp, M3, k, cutoff);

    subtract_matrix(B21, B11, B_tmp, k);
    int **M4 = allocate_matrix(k); strassen_hybrid(A22, B_tmp, M4, k, cutoff);

    add_matrix(A11, A12, A_tmp, k);
    int **M5 = allocate_matrix(k); strassen_hybrid(A_tmp, B22, M5, k, cutoff);

    subtract_matrix(A21, A11, A_tmp, k); add_matrix(B11, B12, B_tmp, k);
    int **M6 = allocate_matrix(k); strassen_hybrid(A_tmp, B_tmp, M6, k, cutoff);

    subtract_matrix(A12, A22, A_tmp, k); add_matrix(B21, B22, B_tmp, k);
    int **M7 = allocate_matrix(k); strassen_hybrid(A_tmp, B_tmp, M7, k, cutoff);
